...
// this for the rounding error, increasing N rounding error increases 0.01
// precision good for N > 8000
#define R_ERROR 0.01

// main routine to calculate DFT
int DFT(int idft, double *xr, double *xi, double *Xr_o, double *Xi_o, int N, int threads);
...
int main(int argc, char *argv[])
{
	...
	// DFT
	int idft = 1;
	DFT(idft, xr, xi, Xr_o, Xi_o, N, threads);
	// IDFT
	idft = -1;
	DFT(idft, Xr_o, Xi_o, xr_check, xi_check, N, threads);
...
{
	int k, n;
	#pragma omp parallel num_threads(threads) private(k, n)
    {
	#pragma omp for
	for (k = 0; k < N; k++)
	{
